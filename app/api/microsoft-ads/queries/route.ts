...
export async function GET() {
  try {
    ...
    const queries = await prisma.query.findMany({
      where: { userId, service: "Microsoft Ads" },
      orderBy: { createdAt: "desc" },
    });
    return NextResponse.json({ queries });
  } catch (error) {
    console.error("GET /api/microsoft-ads/queries error:", error);
...
export async function POST(request: Request) {
  try {
    ...
    console.error("POST /api/microsoft-ads/queries error:", error);
    return new NextResponse("Internal Server Error", { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    console.error("PUT /api/microsoft-ads/queries error:", error);
    return new NextResponse("Internal Server Error", { status: 500 });
  }
}
